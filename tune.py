...
BINARY = ROOT / "bin" / "chess_montecarlo"
TOURNAMENT = ROOT / "data" / "candidates2024.json"
HYPERPARAMETERS = ROOT / "configs" / "hyperparameters.json"
DB = ROOT / "optuna.db"
...
def evaluate(
    params: dict, games: list[dict], hyper_base: dict, trial: optuna.Trial
) -> float:
    ...
    for i, r in enumerate(rounds):
        ...
        try:
            # Matches C++ signature: ./chess_montecarlo [hyperparams] [tournament] [round]
            proc = subprocess.run(
                [str(BINARY), str(tmp), str(TOURNAMENT), str(r)],
                capture_output=True,
                text=True,
                timeout=120,
            )
        ...
        # Evaluate ALL known games that happen at or after the current simulation round
        for g in [g for g in games if g["round"] >= r]:
            ...
            if g_round in preds_by_round and key in preds_by_round[g_round]:
                ...
                # Exponential decay based on distance into the future
                distance = g_round - r
        ...
        # Intermediate report for pruning bad trials early
...
def objective(trial: optuna.Trial, games: list[dict], hyper_base: dict) -> float:
    """Hyperparameter search space identically mirroring the JSON aesthetic."""

    # ── MAP prior weights ────────────────────────────────────────────────────
    map_priors = {
        # Narrowed to realistic Bayesian updates
    ...
    velocity = {
        # Shifted up because 46 was near the top of the old bound
        "initial_white_adv": trial.suggest_float(
            "initial_white_adv", 25.0, 65.0, step=1.0
        ),
        # Floored at 0.70; 0.50 decays history way too fast for classical
        "velocity_time_decay": trial.suggest_float(
            "velocity_time_decay", 0.70, 1.0, step=0.05
        ),
        # Floored at 0.10 so it always uses some historical momentum
    ...
    blending = {
        # Negative weights rejected by the engine. Shifted bounds up based on 0.45/0.50 results.
        ...
        # Floored at 0.05 so it never completely turns off, capped at 0.30
        "color_bleed": trial.suggest_float("color_bleed", 0.05, 0.30, step=0.05),
    }

    # ── Draw model (Baseline Nu) ─────────────────────────────────────────────
    draw = {
    ...
    # ── Hybrid Aggression & Game Theory ──────────────────────────────────────
    aggression = {
        # Massive ceiling raise. 9.5 was hitting the old limit of 10.0.
        "agg_prior_weight": trial.suggest_float(
            "agg_prior_weight", 2.0, 20.0, step=0.5
        ),
        # Raised White's ceiling to 0.85 (0.65 hit the limit).
        ...
        # Floored at 0.05 so game-theory standings tension is always active
    ...
    return evaluate(params, games, hyper_base, trial)
...
def main():
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="continue an existing study from optuna.db",
    )
    args = parser.parse_args()

    if not BINARY.exists():
        raise FileNotFoundError(f"Binary not found: {BINARY}. Build first.")

    hyper_base = load_jsonc(HYPERPARAMETERS)
    games = known_games(load_jsonc(TOURNAMENT))
    ...
    if not study.trials:
        baseline = {
            "prior_weight_known": hyper_base.get("prior_weight_known", 0.5),
            "prior_weight_sim": hyper_base.get("prior_weight_sim", 3.0),
            "initial_white_adv": hyper_base.get("initial_white_adv", 35.0),
            "velocity_time_decay": hyper_base.get("velocity_time_decay", 0.95),
            "lookahead_factor": hyper_base.get("lookahead_factor", 1.0),
            "rapid_form_weight": hyper_base.get("rapid_form_weight", 0.25),
            "blitz_form_weight": hyper_base.get("blitz_form_weight", 0.15),
            "color_bleed": hyper_base.get("color_bleed", 0.20),
            "classical_nu": hyper_base.get("classical_nu", 2.5),
            "rapid_nu": hyper_base.get("rapid_nu", 1.5),
            "blitz_nu": hyper_base.get("blitz_nu", 0.8),
            "agg_prior_weight": hyper_base.get("agg_prior_weight", 3.0),
            "default_aggression_w": hyper_base.get("default_aggression_w", 0.25),
            "default_aggression_b": hyper_base.get("default_aggression_b", 0.15),
            "standings_aggression": hyper_base.get("standings_aggression", 0.15),
        }
        study.enqueue_trial(baseline)

    study.optimize(
        lambda t: objective(t, games, hyper_base),
